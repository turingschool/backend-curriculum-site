...
```
As a developer, I would like to make a rake task such as "rake data:import" which populates my database with
the PGDUMP and CSV data in a reproducible way.
...
- Be invocable through Rake, i.e. you should be able to run `bundle exec rake <your_rake_task_name>` from the command line
- Convert all prices before storing. Prices in the CSV files are in cents, therefore you will need to transform them to dollars. (`12345` becomes `123.45`)
