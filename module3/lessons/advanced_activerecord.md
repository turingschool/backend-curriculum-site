...
`unscoped` removes all scopes that occur before it is called but leaves the scopes that occur after it in tact. This applies to both `default_scope` and `scope`. A tradeoff with this approach is if we were merging `Transaction.successful` in many queries adding `default_scope` might break them. That said, calling `Transaction.order(id: :asc)` all over our codebase might be equally bad. YMMV but be aware of the tradeoffs.
